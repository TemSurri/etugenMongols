...
export const GalleryTimeline = memo(function GalleryTimeline({
  items,
  copy,
}: {
...
const GalleryTimelineItem = memo(function GalleryTimelineItem({
  item,
  index,
  copy,
}: {
...
function TimelineDot() {
  return (
    ...
    >
      <div className="h-3 w-3 bg-[#d7c896] shadow-[0_0_0_6px_rgba(30,32,24,0.9)]" />
    </div>
  );
}
...
const TimelineGalleryCard = memo(function TimelineGalleryCard({
  item,
  copy,
  index,
}: {
  ...
  return (
    <Link
      ...
      className="group block overflow-hidden bg-[#fffaf0] text-[#27301d] shadow-[0_18px_50px_rgba(0,0,0,0.3)] transition-transform duration-300 motion-safe:hover:-translate-y-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#e1d2a6]"
    >
    ...
    >
      <div
        className={[
          ...
          isLeftSide ? "text-left" : "md:ml-auto md:text-right",
        ].join(" ")}
      >
        <p className="text-[10px] font-bold uppercase tracking-[0.24em] text-[#d7c896]">
          {copy.pastEvent}
        ...
        <Link
          to={item.link}
          className="mt-6 inline-flex border-b border-[#e1d2a6]/50 pb-1 text-[10px] font-bold uppercase tracking-[0.2em] text-[#e1d2a6] transition-colors hover:border-[#fffaf0] hover:text-[#fffaf0] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#e1d2a6]"
        >
          {copy.viewAlbum} →
        </Link>
      </div>
