...
export function RegistrationsSection({ copy }: RegistrationsSectionProps) {
  const { lang } = useLanguage();

  const { registrations, loading, error, reload } = useUserRegistrations();
  ...
  const groupedRegistrations = useMemo(() => {
    ...
    for (const registration of registrations) {
      ...
      if (existing) {
        ...
        grouped.set(eventId, [registration]);
      }
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!loading && !error && groupedRegistrations.length === 0 && (
