// src/sections/users/registrations/copy/registrationsCopy.ts

export const registrationsCopy = {
  en: {
    title: "Programs & Registrations",

    subtitle: "View the events and programs connected to your account.",

    section: {
      title: "Your Registrations",

      description:
        "View the events you have registered for and the attendees connected to your account.",

      loading: "Loading your registrations...",

      error: "We couldn't load your registrations right now.",

      retry: "Try again",

      emptyTitle: "No registrations yet",

      emptyBody: "Events and programs you register for will appear here.",

      attendees: "Attendees",

      registration: "registration",

      registrations: "registrations",

      registered: "Registered",

      cancelled: "Cancelled",

      eventDate: "Event date",

      location: "Location",
    },
  },

  mn: {
    title: "Хөтөлбөр ба бүртгэл",

    subtitle:
      "Таны бүртгэлтэй холбоотой арга хэмжээ болон хөтөлбөрүүдийг хараарай.",

    section: {
      title: "Таны бүртгэлүүд",

      description:
        "Таны бүртгүүлсэн арга хэмжээ болон таны бүртгэлтэй холбоотой оролцогчдыг хараарай.",

      loading: "Таны бүртгэлүүдийг ачаалж байна...",

      error: "Бүртгэлүүдийг одоогоор ачаалж чадсангүй.",

      retry: "Дахин оролдох",

      emptyTitle: "Одоогоор бүртгэл алга",

      emptyBody:
        "Таны бүртгүүлсэн арга хэмжээ болон хөтөлбөрүүд энд харагдана.",

      attendees: "Оролцогчид",

      registration: "бүртгэл",

      registrations: "бүртгэл",

      registered: "Бүртгэлтэй",

      cancelled: "Цуцлагдсан",

      eventDate: "Арга хэмжээний огноо",

      location: "Байршил",
    },
  },
} as const;
