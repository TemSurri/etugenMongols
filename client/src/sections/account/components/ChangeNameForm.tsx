...
export function ChangeNameForm({
  ...
  const { submit, loading, error, success } = useChangeName({
    invalidNameMessage: copy.invalidName,
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-6 space-y-4">
      ...
      {success && <p className="text-sm text-green-700">{copy.nameSuccess}</p>}
      ...
      <div className="flex justify-end gap-3 pt-2">
        ...
        <button
          type="submit"
          disabled={loading || !newFirstName.trim() || !newLastName.trim()}
        ...
        >
          {loading ? copy.changingName : copy.save}
        </button>
      </div>
    </form>
  );
...
function NameInput({ label, value, onChange, autoComplete }: NameInputProps) {
  return (
