...
type Props = { event: UpcomingEventItem; copy: EventsCopy; index: number };
    ...
    >
      ...
      <div className="absolute inset-x-0 bottom-0 p-5 text-[#fffaf0] sm:p-6">
        <p className="text-[10px] font-medium leading-5 text-[#eee0b7]">
          {event.date}
          {event.time && <><span className="mx-2 text-[#eee0b7]/45">·</span>{event.time}</>}
        </p>
        <h2 className="mt-2 line-clamp-2 text-xl font-normal leading-tight tracking-tight sm:text-2xl">{event.title}</h2>
        {event.location && <p className="mt-2 line-clamp-1 text-[11px] leading-5 text-[#fffaf0]/72">{event.location}</p>}
        <p className="mt-4 text-[9px] font-semibold uppercase tracking-[0.16em]">
          {copy.viewEvent}<span aria-hidden="true" className="ml-2 inline-block transition-transform duration-200 motion-safe:group-hover:translate-x-1">→</span>
        </p>
      </div>
    </Link>
  );
