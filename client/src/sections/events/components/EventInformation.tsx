...
function YearlyEvent({ title, body }: { title: string; body: string }) {
  return (
    <article>
      <h3 className="text-lg font-normal leading-tight md:text-xl">{title}</h3>
      <p className="mt-3 max-w-xl text-sm leading-7 text-[#cfc8b5]">{body}</p>
    </article>
  );
}
...
function DarkAction({ to, label }: { to: string; label: string }) {
  return (
    <Link
      to={to}
      className="group inline-flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.14em] text-[#fffaf0] transition-colors hover:text-[#d6ba72] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#fffaf0]/50 focus-visible:ring-offset-4 focus-visible:ring-offset-[#303824]"
    >
      {label}
      <span
        aria-hidden="true"
...
function EventInformation({ copy }: { copy: EventsCopy }) {
  return (
    <motion.section
      ...
      aria-labelledby="yearly-events-title"
    ...
    >
      <div className="mx-auto w-full max-w-[660px]">
        ...
        <p className="mt-4 max-w-xl text-sm leading-7 text-[#d8d1bf]">
          {copy.yearlyBody}
        </p>
        <div className="mt-7 space-y-7 border-t border-[#fffaf0]/15 pt-7">
          <article>
            ...
            <p className="mt-3 max-w-xl text-sm leading-7 text-[#cfc8b5]">
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="font-medium text-[#fffaf0] underline decoration-[#d6ba72]/60 underline-offset-4 transition-colors hover:text-[#d6ba72] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#fffaf0]/50"
              >
                {copy.naadamLink}
              </a>
              {copy.naadamAfter}
          ...
          <YearlyEvent
            title={copy.performancesTitle}
            body={copy.performancesBody}
          />
        </div>
